  ...
  async search() {
    if (this.date) {
      const formattedDate = this.date
        ? (this.date as Date).toISOString().slice(0, 7)
        : ""; // 将日期转换为 'YYYY-MM' 格式
      console.log("搜索日期:", formattedDate);

      this.listOfData = await firstValueFrom(
        this.attendanceService.get_attendance_by_date(formattedDate)
      );

      this.users_node = await firstValueFrom(
        this.employeeService.get_employee()
      );
      ...
      this.listOfData = await firstValueFrom(
        this.attendanceService.get_attendance()
      );
  ...
  handleOk() {
    if (this.validateAddForm.valid) {
      ...
      this.attendanceService
        .post_attendance(attendance_data)
        .subscribe((res) => {
          this.refresh_data();
        });
    } else {
